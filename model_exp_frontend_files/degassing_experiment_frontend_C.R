...
parLapply(cl = cl, pars_list, fun = function(df_entry){
  ...
  CO2_ppm_start <- 800 # This sets the starting CO2 conentration for the model experiment
  t <- -200e+6 # This sets the geologic time period and paleogeography considered, current options are: -250e+6 for the Permian-Triassic, -200e+6 for the Triassic Jurassic, -55e+6 for the Paleocene-Eocene, 0 for the present day configuration
  degass_mass <- 30000e+15 # Set the amount of C degassing during the LIP Period, this should be given in grams
  degass_duration <- 2 # Number of time steps over which degassing occurs
